...
// Usaremos este token en el encabezado Authorization: Bearer
define('ZABBIX_API_TOKEN', '23c5e835efd1c26742b6848ee63b2547ce5349efb88b4ecefee83fa27683cb9a'); 
// El nombre del trigger debe coincidir EXACTAMENTE con el de tu plantilla ICMP.
define('ICMP_TRIGGER_NAME_SEARCH', 'Unavailable by ICMP ping');
// -----------------------------

/**
 * Función para realizar peticiones a la API de Zabbix (JSON-RPC).
 * 🛠️ CORRECCIÓN CLAVE: El token se envía vía el encabezado 'Authorization: Bearer'.
...
function zabbix_api_request(string $method, array $params = []): array {
    $data = [
        ...
        'params' => $params,
        'id' => 1
        // Se ELIMINA 'auth' del cuerpo JSON para usar el encabezado Bearer.
    ];
    ...
    curl_setopt_array($ch, [
        ...
        CURLOPT_POST => true, // Usamos POST, que es lo esperado para JSON-RPC con cuerpo
        CURLOPT_POSTFIELDS => $json_payload,
        CURLOPT_HTTPHEADER => [
            'Content-Type: application/json',
            // 💡 APLICACIÓN DE AUTENTICACIÓN BEARER EN EL ENCABEZADO:
...
try {
    ...
    // Asegurarse que la fecha final incluye todo el último día (23:59:59)
    ...
    // 2.1 Get ID of the selected host group
    ...
    // 2.2 Get Zabbix hosts belonging to the specified group, excluding templates
    ...
    // 2.3 Get IPs of host interfaces
    ...
    // 2.4 Get "ICMP Unavailable" triggers for the valid hosts
    ...
    if (!is_dir($output_dir)) {
        if (!mkdir($output_dir, 0777, true) && !is_dir($output_dir)) {
            throw new Exception("No se pudo crear el directorio de salida: $output_dir");
        }
    }
    ...
    error_log("Error en el script de disponibilidad: " . $e->getMessage());
}
?>