    ...
    async function loadConfigData() {
        try {
            const [fpResp, posResp] = await Promise.all([
                fetch('php/api.php?action=get_floorplans'),
                fetch('php/api.php?action=get_ap_positions')
            ]);
...
async function saveCurrentState() {
    ...
    try {
        const fpResp = await fetch('php/api.php?action=save_floorplan', { method: 'POST', body: formData });
        ...
        if (fpRes.success) {
            ...
            const apResp = await fetch('php/api.php?action=save_all_ap_positions', { method: 'POST', body: apFormData });
...
async function handleImageUpload(e) {
    ...
    try {
        const resp = await fetch('php/api.php?action=upload_image', { method: 'POST', body: uploadData });
...
async function deleteFloor(id) {
    const resp = await fetch(`php/api.php?action=delete_floorplan&id=${id}`);
