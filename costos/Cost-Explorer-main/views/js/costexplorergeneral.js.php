...
<script>
	const view = {
		...
		init({filter_options, refresh_url, refresh_interval, filter_defaults}) {
			...
			url.setArgument('action', 'costexplorer.view.refresh');
		...
		initFilter(filter_options) {
			...
			this.filter.on(TABFILTER_EVENT_URLSET, () => {
				const url = new Curl();
				url.setArgument('action', 'costexplorer.view.csv');
		...
		getCurrentResultsTable() {
			return document.getElementById('costexplorer-table');
		},
		...
		const table = this.getCurrentResultsTable();
		if (table) {
			table.classList.add('is-loading', 'is-loading-fadein', 'delayed-15s');
		}
	},
	...
	clearLoading() {
		const table = this.getCurrentResultsTable();
		if (table) {
			table.classList.remove('is-loading', 'is-loading-fadein', 'delayed-15s');
		}
	},
	...
	refreshDebug(debug) {
		const debugBlock = this.getCurrentDebugBlock();
		if (debugBlock) {
			debugBlock.replaceWith(
		...
		refreshResults() {
			...
			refresh_url.setArgument('action', 'costexplorer.view.refresh');
